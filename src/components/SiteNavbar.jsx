      ...
      {/* <header className="fixed top-0 w-full z-50 h-20 border-b border-surface-variant/20 bg-surface/95 backdrop-blur-sm"> */}
        <div className="mx-auto flex h-full w-full max-w-container-max items-center px-4 md:px-8 lg:px-10">
          ...
          >
            ...
            >
              {t('about')}
              <span className={`material-symbols-outlined text-base transition-transform duration-200 ${openMenu === 'about' ? 'rotate-180 text-vibrant-orange' : ''}`}>expand_more</span>
            </button>
            <div className={`absolute left-0 top-full mt-3 min-w-72 rounded-xl bg-surface/95 backdrop-blur-sm origin-top transition-all duration-200 ${openMenu === 'about' ? 'opacity-100 scale-100 pointer-events-auto' : 'opacity-0 scale-95 pointer-events-none'}`}>
              <div className="px-3 py-2 border-b border-surface-variant/40">
                <span className="text-xs font-bold tracking-[0.12em] uppercase text-vibrant-orange">{t('aboutUs')}</span>
              </div>
              <div className="py-1">
              {aboutDropdownItems.map((item) => (
                ...
                >
                  ...
                  <span className="material-symbols-outlined text-base opacity-60">arrow_outward</span>
                </a>
              ))}
              </div>
            </div>
          ...
          >
            ...
            >
              {t('programs')}
              <span className={`material-symbols-outlined text-base transition-transform duration-200 ${openMenu === 'programs' ? 'rotate-180 text-vibrant-orange' : ''}`}>expand_more</span>
            </button>
            <div className={`absolute left-0 top-full mt-3 min-w-80 rounded-xl bg-surface/95 backdrop-blur-sm origin-top transition-all duration-200 ${openMenu === 'programs' ? 'opacity-100 scale-100 pointer-events-auto' : 'opacity-0 scale-95 pointer-events-none'}`}>
              <div className="px-3 py-2 border-b border-surface-variant/40">
                <span className="text-xs font-bold tracking-[0.12em] uppercase text-vibrant-orange">{t('programs')}</span>
              </div>
              <div className="py-1">
          ...
          <a href="/get-involved" onClick={(event) => handleNavigate(event, '/get-involved')} className={activePage === 'get-involved' ? activeClass : linkClass}>{t('join')}</a>
          <a href="/our-impact" onClick={(event) => handleNavigate(event, '/our-impact')} className={activePage === 'our-impact' ? activeClass : linkClass}>{t('impact')}</a>
          <a href="/gallery" onClick={(event) => handleNavigate(event, '/gallery')} className={activePage === 'gallery' ? activeClass : linkClass}>{t('gallery')}</a>
          <a href="/contact-us" onClick={(event) => handleNavigate(event, '/contact-us')} className={activePage === 'contact' ? activeClass : linkClass}>{t('contact')}</a>
          </nav>
